...
bool checkBaoshiAfterChange(int a[8][8]){//判断交换后全局内是否可以消去，这个函数供下面这个函数使用，调用下面这个
    bool flag = false;
    for(int i = 0; i < 8; i++){
        for(int j = 0; j < 8; j++){
...
bool checkBaoshi(int a[8][8]){//判断交换后是否可以消除，返回true or false
    int b[8][8];
    int temp;
    bool flag = false;
    fuzhiArray(a,b);
...
void  bian0(int a[8][8],int x,int y){//将一块联通区变为0，被下面的函数使用
    int value = a[x][y];
    a[x][y] = 0;
